...
module.exports.run = async (bot, message, args) => {
    ...
    if(!message.member.roles.find(r => r.name === "ADMINISTRATOR") || message.member.roles.find(r => r.name === "Moderador")) return message.reply("**Você não tem permissão para isso.**");
    ...
    let muteembed = new Discord.RichEmbed()
        ...
        .setTimestamp()
    msg.guild.channels.get(`695753295628599407`).send(muteembed);

    await (tomute.addRole(muterole.id));

    setTimeout(function () {
        tomute.removeRole(muterole.id);
        msg.guild.channels.get(`695753295628599407`).send(`<@${tomute.id}> aprendeu a falar direito e foi desmutado.`);
    }, ms(mutetime));
